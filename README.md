<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0EA5E9,100:22C55E&height=190&section=header&text=Adel%20Mohsen&fontSize=52&fontAlignY=35&desc=Automation%20%7C%20Data%20%7C%20ML%20%7C%20Full-Stack&descAlignY=62" />
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/adel-mohsen-9702aa15b/">
    <img src="https://img.shields.io/badge/LinkedIn-Adel%20Mohsen-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:AdelMohsen145@gmail.com">
    <img src="https://img.shields.io/badge/Email-AdelMohsen145%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://github.com/adel145/Adel_Portfolio-">
    <img src="https://img.shields.io/badge/Portfolio-GitHub%20Repo-8B5CF6?style=for-the-badge&logo=github&logoColor=white" />
  </a>
  <a href="https://www.crystalliaworld.com/">
    <img src="https://img.shields.io/badge/Website-Crystallia%20Store-22C55E?style=for-the-badge" />
  </a>
</p>







<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=16&duration=2500&pause=600&center=true&vCenter=true&width=640&lines=CS+B.Sc.+%7C+Israel;Building+automation%2C+data+pipelines+%26+ML+projects;React+%2B+TypeScript+%2B+Python+%2B+SQL+%2B+Git;Open+to+Automation%2FData%2FML+roles" />
</p>

---

## About me
- Computer Science B.Sc. (Israel)  
- I build **automation**, **data/ML** projects, and **full-stack** products.
- I like clean architecture, measurable results, and shipping fast.

## Focus
- **Automation / QA:** CI/CD mindset, scripts, reliability, testing habits  
- **Data:** SQL, ETL thinking, data cleaning, analysis-ready pipelines  
- **ML:** baseline → evaluation → iteration (practical, not hype)

---
<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/adel145/adel145/output/github-contribution-grid-snake-dark.svg" />
    <img alt="snake animation" src="https://raw.githubusercontent.com/adel145/adel145/output/github-contribution-grid-snake.svg" />
  </picture>
</p>

## Featured projects

### 🔥 Miktsoan (Final Project 2026) — Frontend
**Live demo:** https://adel145.github.io/FinalProject2026/  
**Repo:** https://github.com/adel145/FinalProject2026  
**Stack:** React + TypeScript + Tailwind + shadcn/ui + Framer Motion + i18n (EN/HE)

**Short deploy text (copy-paste):**  
Deployed on **GitHub Pages** (Vite). Auto-published from `main` to the Pages site after each update.

---

### 🤖 WaiterTip (Tips Predictor) — ML mini project
**Repo:** https://github.com/adel145/Tips_Predictor  
A simple Python ML project for predicting tips from a dataset.

---

### 🎮 3D Portfolio (Full-Stack)
**Repo:** https://github.com/adel145/Adel_Portfolio-  
My strongest “showcase” project: interactive portfolio + full-stack vibe.

---

### 💎 Crystallia — Jewelry E-commerce Store (Built & Relaunched)
**Live site:** https://www.crystalliaworld.com/  
Built and customized my own jewelry e-commerce website, focusing on a clean product experience, performance, and SEO-ready structure.


## Tech stack (what I actually use)
**Languages:** Python • JavaScript/TypeScript • SQL • C# • Java/C++  
**Web:** React • Node.js • Express • REST APIs • Tailwind  
**Data/ML:** Pandas • NumPy • scikit-learn (and practical evaluation)  
**Tools:** Git/GitHub • Linux • Postman

---

## GitHub stats
<p align="center">
  <img height="165" src="https://github-readme-stats.vercel.app/api?username=adel145&show_icons=true&hide_border=true" />
  <img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=adel145&layout=compact&hide_border=true" />
</p>

<p align="center">
  <img src="https://streak-stats.demolab.com?user=adel145&hide_border=true" />
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=adel145&hide_border=true" />
</p>

---

## Let’s connect
- LinkedIn: https://www.linkedin.com/in/adel-mohsen-9702aa15b/
- Email: AdelMohsen145@gmail.com
- Live site: https://www.crystalliaworld.com/ 
